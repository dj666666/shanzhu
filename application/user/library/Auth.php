...
class Auth extends \fast\UserAuth
{
    ...
    public function login($username, $password, $keeptime = 0,$googlecode)
    {
        ...
        //判断是否开启谷歌验证
        if (Config::get('fastadmin.login_checkusergoogle')) {
        ...
        //判断是否限制ip登入
        if (Config::get('fastadmin.loginip_checkuserlogin')) {
            $ip = request()->ip();
    ...
    public function isLogin()
    {
        ...
        //判断管理员IP是否变动
        if (Config::get('fastadmin.loginip_checkuser')) {
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        // 必须将结果集转换为数组
        $ruleList = collection(\app\user\model\AuthRule::where('status', 'normal')
            ->where('ismenu', 1)
            ->order('weigh', 'desc')
            ->cache("__usermenu__")
            ->select())->toArray();
