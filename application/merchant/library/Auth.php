...
class Auth extends \fast\MerchantAuth
{
     ...
     * @return  boolean
     */
    public function login($username, $password, $keeptime = 0,$googlecode)
    {
        ...
        //判断是否开启谷歌验证
        if (Config::get('fastadmin.login_checkmerchantgoogle')) {
        ...
        //判断是否限制ip登入
        if (Config::get('fastadmin.loginip_checkmerchantlogin')) {
            $ip = request()->ip();
    ...
    public function isLogin()
    {
        ...
        //判断管理员IP是否变动
        if (Config::get('fastadmin.loginip_checkmerchant')) {
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        // 必须将结果集转换为数组
        $ruleList = collection(\app\merchant\model\AuthRule::where('status', 'normal')
            ->where('ismenu', 1)
            ->order('weigh', 'desc')
            ->cache("__merchantmenu__")
            ->select())->toArray();
