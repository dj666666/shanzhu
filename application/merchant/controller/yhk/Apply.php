 ...
 * @icon fa fa-circle-o
 */
class Apply extends MerchantBackend
{

    /**
     * Apply模型对象
     * @var \app\merchant\model\yhk\Apply
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\merchant\model\yhk\Apply;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            //找出商户余额
            $sh_money = Db::name('merchant')->where(['id'=>$this->auth->id])->field('money,rate_money,block_money')->find();

            $result = array("total" => $total, "rows" => $list, "extend" => [
                'money' => $sh_money['money'],
                'rate_money' => $sh_money['rate_money'],
                'block_money' => $sh_money['block_money'],
                ]);
