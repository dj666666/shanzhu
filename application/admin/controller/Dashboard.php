...
class Dashboard extends Backend
{
    ...
    public function index()
    {
        ...
        $today_fees = Db::name('order')->where(['status'=>1])->whereTime('createtime', 'today')->sum('fees');//今日手续费
        $yesterday_fees = Db::name('order')->where(['status'=>1])->whereTime('createtime', 'yesterday')->sum('fees');		//昨日手续费
        $all_fees = Db::name('order')->where(['status'=>1])->sum('fees');		//总手续费
